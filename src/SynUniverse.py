...
SynConf = namedtuple('SynUniverse','profile band_freq band_noise inten_group inten_values iso_abun base_abun base_CO spl_cons')
...
class SynCube:
	...
	def __init__(self,log,name,conf,spec):
		...
		log.write('  -> Cube size: '+str(len(self.x_axis))+' x '+str(len(self.y_axis))+' x '+str(len(self.v_axis))+' \n')
		self.band='NO_BAND'
	...
	def animate(self,inte,rep=True):
      ...
      """
		...
		ani = animation.FuncAnimation(fig, self._updatefig, frames=range(len(self.v_axis)), interval=inte, blit=True,repeat=rep)
		plt.show()
		...
		for struct in self.structs:
			...
			for mol in struct.intens:
				...
				for i in range(len(mlin)):
					...
					log.write('E: '+mlin[i]['chemicalname']+' ('+mlin[i]['molformula']+') at '+str(mlin[i]['frequency'])+' Mhz, T = exp(-|'\
					...
					for idx in range(window[0],window[1]):
						cube.data[:,:,idx]+= tcub*temp*exp((-0.5*(cube.v_axis[idx] -  freq/1000.0)**2)/(sigma**(2*shape)))
