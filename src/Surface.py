...
def genSurface(form,alpha_axis,delta_axis):
        ...
        Xc = alpha_mesh.flatten() - self.alpha * np.ones(len(alpha_axis) * len(delta_axis))
        Yc = delta_mesh.flatten() - self.delta * np.ones(len(alpha_axis) * len(delta_axis))
