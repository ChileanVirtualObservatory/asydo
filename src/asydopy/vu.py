...
def gen_line(spe_form,freq,axis):
    ...
    - ("skew",fwhm,alpha) : a skew-normal distribution, where fwhm is 
              ...
              distribution, if alpha < 0, the distriburion is leftbiased
...
class SpectralCube:
    ...
    def __init__(self, log, name, alpha, delta, freq, ang_res, ang_fov, spe_res, spe_bw, band_freq=ALMA_bands,band_noises=ALMA_noises):
        """ 
    ...
    def animate(self, inte, rep=True):
        ...
        ani = animation.FuncAnimation(fig, self._updatefig, frames=range(len(self.freq_axis)), interval=inte, blit=True,repeat=rep)
        plt.show()
...
class IMCM(Component):
    ...
    def __init__(self, log,dbpath, mol_list, temp, spa_form, spe_form, z_grad,z_base=0.0, abun_max=10 ** -5, abun_min=10 ** -6, abun_CO=1.0, iso_abun=default_iso_abundance):
